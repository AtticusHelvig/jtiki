...
class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
    ...
    public Object visitGetExpr(Expr.Get expr) {
        ...
        if (object instanceof TikiInstance) {
            return ((TikiInstance) object).get(expr.name);
        }
    ...
    public Object visitSetExpr(Expr.Set expr) {
        ...
        Object value = evaluate(expr.value);
        ((TikiInstance) object).set(expr.name, value);
        return value;
    }
    ...
    public Object visitSuperExpr(Expr.Super expr) {
        ...
        TikiClass superclass = (TikiClass) environment.getAt(distance, "super");

        // NOTE: Despite the method being in the superclass 'this' should still
        // refer to the same current object we're using.
        TikiInstance object = (TikiInstance) environment.getAt(distance - 1, "this");

        TikiFunction method = superclass.findMethod(expr.method.lexeme);
    ...
    public Void visitClassStmt(Stmt.Class stmt) {
        ...
        if (stmt.superclass != null) {
            ...
            if (!(superclass instanceof TikiClass)) {
        ...
        TikiClass tikiClass = new TikiClass(stmt.name.lexeme, (TikiClass) superclass, methods);

        if (stmt.superclass != null) {
            environment = environment.enclosing;
        }
        environment.assign(stmt.name, tikiClass);
        return null;
    }
